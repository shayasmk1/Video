...
class ChannelApiController extends ApiBaseController
{
    ...
    {
        $channels = $this->channel->getMyChannels($request->get('id'));
        return $this->respondWithCollection($channels, new ChannelTransformer());
    }
    ...
    public function currentChannelPosition(Request $request, $channelID, $videoID)
    {
        if(!$request->exists('data'))
        {
            return $this->errorWrongArgs(['No Input found']);
        ...
        $video = $this->channel->findWhere(['uuid' => $channelID])->first();
        if(!$video)
        {
            return $this->errorWrongArgs(['Channel not found']);
        }
        
        $validation = $this->channelValidator->channelLogInsert($data);
        if($validation)
        {
            return $this->errorWrongArgs($validation['errors']);
        }
        ...
        $log = $this->channelLogRepo->insertChannelDetails($data);
        return $this->respondWithBoolean($log, new ChannelLogTransformer);
    }
}
