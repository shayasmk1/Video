...
class ReportsApiController extends ApiBaseController
{
    ...
    public function userStatisticsOverTags(Request $request)
    {
        $report = $this->user->getUserStatisticsOverTags($request->get('id'));
    ...
    {
        $data = $request->get('data');
        $report = $this->videoHistory->videoTraffic($videoID, $data['from_date'], $data['to_date'], $request->get('id'));
        return response()->json($report, 200);
    }
    
    public function userStatisticsOverTagsChannel(Request $request)
    {
        $report = $this->user->getUserStatisticsOverTagsChannel($request->get('id'));
        return response()->json($report, 200);
    }
    ...
    {
        $data = $request->get('data');
        $report = $this->videoHistory->channelTraffic($channelID, $data['from_date'], $data['to_date'], $request->get('id'));
        return response()->json($report, 200);
    }
}